...
export default {
  ...
  methods: {
    ...
    getCurrectPage1(currect) {
      console.log(currect);
      this.getCommentPage({ row, offset: currect });
    },
    ...
    allPass({ idListStr, changeFlag }) {
      this.pass_offline_all_comment({ idListStr, changeFlag }).then(res => {
        console.log(res);
        if (res.ok) {
          this.getReportList({
            offset: 1,
    ...
    pass({ idListStr, changeFlag }) {
      this.pass_offline_comment({ idListStr, changeFlag }).then(res => {
        if (res.data.ok) {
          this.$Message.info("操作成功");
        } else {
          this.$Message.info("操作失败");
